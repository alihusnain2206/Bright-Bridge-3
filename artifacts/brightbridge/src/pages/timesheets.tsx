        ...
        <div className="lg:col-span-1 space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Select client, view mode and employee context.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  {(["all", "employee"] as ViewMode[]).map((mode) => (
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 p-2 rounded border text-sm font-medium transition-colors ${
                        viewMode === mode ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:bg-muted"
                      }`}
                    >
              ...
              <div className="space-y-2">
                <Label>Employee {viewMode === "all" && <span className="text-muted-foreground text-xs">(JWT context)</span>}</Label>
              ...
              {selectedClient && (
                <div className="p-3 bg-muted/50 rounded-lg border border-border text-xs space-y-1">
                  <div className="font-medium">{selectedEmployee?.name ?? "All Employees"}</div>
                  <div className="text-muted-foreground">{selectedEmployee?.roleName ?? "Manager view"} · {selectedClient.name}</div>
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {events.length === 0 ? (
                <div className="px-4 pb-4 text-xs text-muted-foreground text-center py-6">Timesheet events will appear here.</div>
              ) : (
          ...
          <Card className="h-full flex flex-col" style={{ minHeight: 620 }}>
            <CardHeader className="py-4">
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                {selectedEmployee && viewMode === "employee" && <span className="text-sm font-normal text-muted-foreground ml-1">— {selectedEmployee.name}</span>}
              </CardTitle>
            </CardHeader>
              ...
              {!accessToken && (
                <div className="absolute inset-0 flex items-center justify-center bg-muted/30 z-10">
                  <div className="text-center text-muted-foreground max-w-xs p-6">
                    ...
                    <p className="text-sm font-medium">Select a client and click Launch to load timesheets.</p>
                  </div>
                </div>
              )}
