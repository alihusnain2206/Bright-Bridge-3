...
function FilterSelect({ value, onChange, children }: {
  ...
  return (
    <select
      className="h-9 rounded-md border border-gray-200 text-sm px-2.5 bg-white text-gray-700 min-w-[140px] focus:outline-none focus:ring-2 focus:ring-[#0EA5C9]/30"
      value={value}
...
export default function PeopleDirectoryPage() {
  ...
  const { data: deptData } = useQuery<{ departments: Department[] }>({
    queryKey: ["people-dir-departments", companyId],
  ...
  const filtered = useMemo(() => {
    let list = employees.filter(e => e.status !== "terminated" || filterStatus === "terminated");
  ...
  }, [employees, search, filterStatus, filterDept, filterType, filterComp, sortCol, sortDir]);
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <FilterSelect value={filterType} onChange={v => { setFilterType(v); setPage(1); }}>
            ...
            {empTypes.map(t => (
              <option key={t} value={t}>{EMP_TYPE_LABELS[t] ?? t}</option>
            ))}
          </FilterSelect>
          {/* Compliance */}
          <FilterSelect value={filterComp} onChange={v => { setFilterComp(v); setPage(1); }}>
          ...
          {anyFilter && (
            <button className="text-xs text-[#0EA5C9] hover:underline whitespace-nowrap" onClick={clearAll}>
              Clear all
            </button>
          )}
        </div>
        <p className="text-xs text-gray-400 mt-2">
          Showing <span className="font-medium text-gray-600">{filtered.length}</span> employee{filtered.length !== 1 ? "s" : ""}
        </p>
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {!isLoading && pageEmployees.map(emp => {
                ...
                const startFmt = emp.startDate
                  ...
                  : "—";
                const empTypeFmt = EMP_TYPE_LABELS[emp.employmentType] ?? emp.employmentType ?? "—";
                return (
