        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="rounded-xl border p-5 space-y-4" style={PANEL}>
            <div>
              ...
              <p className="text-white/50 text-xs">Select a client and employee to load their schedule.</p>
            </div>
            ...
            {selectedClient && (
              <div className="p-3 rounded-lg text-xs space-y-0.5 border" style={PANEL_INNER}>
                <div className="font-semibold text-white">{selectedEmployee?.name ?? "No employee selected"}</div>
                <div className="text-white/50">{selectedEmployee?.roleName ?? ""} · {selectedClient.name}</div>
                <div className="text-white/35">{selectedClient.locationName} · {selectedClient.timezone.replace("America/", "")}</div>
              </div>
            )}
            ...
            {exchangeWarning && <div className="flex items-start gap-2 p-3 rounded-md text-xs text-amber-300 border border-amber-500/30 bg-amber-900/20"><AlertCircle className="h-4 w-4 shrink-0 mt-0.5" /><span>{exchangeWarning}</span></div>}
            {accessToken && !error && (
          ...
          <div className="rounded-xl border" style={PANEL}>
            ...
            {events.length === 0 ? (
              <div className="px-5 py-6 text-xs text-white/35 text-center">Schedule events will appear here.</div>
            ) : (
              ...
              {!accessToken && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 z-10">
                  ...
                  <div className="text-center text-white/50 max-w-xs px-6">
                    <p className="text-sm font-medium text-white/70">Select a client and click Launch to view the schedule.</p>
